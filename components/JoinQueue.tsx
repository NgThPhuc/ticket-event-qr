...
export default function JoinQueue({
  ...
  const handleJoinQueue = async (): Promise<void> => {
    try {
      const result = await joinWaitingList({ eventId, userId });
      if (result?.success) {
        toast.success("You're in the waiting list.", {
          description: "We'll notify you when a ticket is available.",
        });
      }
    } catch (err: unknown) {
      if (err instanceof ConvexError &&
        typeof err.message === "string" &&
        err.message.includes("joined the waiting list too many times")) {
